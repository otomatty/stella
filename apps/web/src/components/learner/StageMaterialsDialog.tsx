/**
 * ステージ教材ダイアログ (Issue #77)。
 *
 * ステージ詳細の「教材をダウンロード」はハンドラの無いスタブだったため、
 * `GET /api/materials?stageId=...` でステージ内の配布資料をまとめて引き、
...
import { downloadLessonMaterial, listStageMaterials } from "@/lib/cms-api";
import { isBackendConfigured } from "@/lib/backend";

interface StageMaterialsDialogProps {
  open: boolean;
...
function groupByLesson(rows: StageMaterialRow[]): MaterialGroup[] {
...
export const StageMaterialsDialog = ({
  open,
  onOpenChange,
  stageId,
  stageTitle,
}: StageMaterialsDialogProps) => {
  const [materials, setMaterials] = useState<StageMaterialRow[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    void listStageMaterials(stageId)
      .then((rows) => {
      ...
      .catch((err: unknown) => {
        if (reqId !== requestIdRef.current) return;
        console.error("[StageMaterialsDialog] fetch failed", err);
        setMaterials([]);
  ...
  }, [open, stageId, backendEnabled]);

  const handleDownload = async (material: StageMaterialRow) => {
    setDownloadingId(material.id);
    try {
      await downloadLessonMaterial(material);
    } catch (err) {
      console.error("[StageMaterialsDialog] download failed", err);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[min(calc(100vw-2rem),640px)]">
        <DialogHeader>
          ...
          <DialogDescription>{stageTitle} の配布資料</DialogDescription>
        </DialogHeader>
          ...
          {!backendEnabled ? (
            ...
            <div className="flex flex-col items-center gap-2 py-12 text-center text-sm text-ink-3">
              ...
              <div className="text-[12.5px]">
                このステージのレッスンに資料が追加されると、 ここからダウンロードできます。
              </div>
            </div>
          ) : (

...
検証内容（30 ステップ）: コース作成 → 公開（draft は受講者に見えないことも確認）→ 受講登録 →
割当プリセットの作成・dry-run・適用（期限が基準日から展開されること / skip と overwrite の差 /
不正・省略した基準日の 400 / 編集後に古い版で適用すると 409 / 上書きしても出自が変わらないこと /
退役した名前の再利用 / 教材削除で空になったプリセットの自動退役）→
...
- [ ] 学習者を受講登録できる（Admin 受講登録画面。受講生を選ぶ → 教材を割り当てる の順。複数名の一括割当も）
- [ ] 「割当プリセット」タブでプリセットを作成でき、受講生を選んで「プリセットを適用」から
      一括登録できる（基準日を変えると期限が変わる / 既存登録は既定でスキップされる）
- [ ] ユーザー招待（単体 / CSV 一括）でプリセットを選ぶと、招待成功と同時に教材が割り当たり、
      受講登録画面にプリセット名のバッジが出る

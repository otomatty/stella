...
function emitCourse(tenantId: Tenant["id"], course: Course) {
  ...
  lines.push(
    `insert into ${isSqlite ? "" : "public."}courses (id, tenant_id, slug, title, category, color, thumbnail_path, duration_hours, description, instructor_name, status${isSqlite ? ", created_at, updated_at" : ""}) values ('${courseUuid}', '${esc(tenantId)}', '${slug}', '${esc(course.title)}', ${course.category ? `'${esc(course.category)}'` : "null"}, ${course.color ? `'${esc(course.color)}'` : "null"}, ${course.thumbnailPath ? `'${esc(course.thumbnailPath)}'` : "null"}, ${course.duration ?? "null"}, ${course.description ? `'${esc(course.description)}'` : "null"}, ${course.enrolledBy ? `'${esc(course.enrolledBy)}'` : "null"}, 'published'${isSqlite ? `, ${nowExpr()}, ${nowExpr()}` : ""}) on conflict (tenant_id, slug) do update set title = excluded.title, category = excluded.category, color = excluded.color, thumbnail_path = excluded.thumbnail_path, duration_hours = excluded.duration_hours, description = excluded.description, instructor_name = excluded.instructor_name, status = excluded.status, updated_at = ${nowExpr()};`,
  );

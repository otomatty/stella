...
**ITのきほん（`courses/it-basics/`）があります。** 3 モジュール / 3 レッスン / 11 トピックで、構成は [courses/it-basics/CURRICULUM.md](courses/it-basics/CURRICULUM.md)。**スキルツリーの入口（中心の星）になる唯一の前提なし講座**で、学び方（スキルツリー・レッスンの型・質問のしかた）と、コンピュータ・インターネットの本当に基本のことばをそろえます。既存の入門講座 10 件（typescript-basics / sql-basics / html-css-basics / git-basics / python-testing-ci-basics / test-design-basics / ai-fluency-basics / fe-kamoku-a / fe-kamoku-b / aws-clf-c02-basics）が `course.json` の `prerequisites` でこの講座の後ろに繋がっており、**この講座に前提を書いてはいけません**（中心の星が閉じるとツリー全体が開かなくなる）。学習対象がことばの理解であってコードではないため、**コード演習は配線していません**（`course.json` に `exercises` を持たない）。手を動かす部分は `practice.md` の「手元で試す」（自分の PC と紙の上）に置き、LMS 上はスライド → まとめ → 確認クイズだけで完走できます。図解 SVG は未作成（`assets/` を持つトピックが無い）。

...
- **画像素材の追加** — 実画面のスクリーンショット(Playground・VS Code)の挿入。講座サムネイルは執筆済み 21 講座に設置済み（準備中 13 講座は未設置）
- **収録** — 162本の動画収録は未着手

...
4. 入力欄に `name="mail"` を足し、ボタンとまとめて `<form action="/apply">` 〜 `</form>` で囲む。ボタンを押すと、行き先のURLの末尾に `?mail=...` と入力内容が付くことを確かめる
5. `name="mail"` だけを消してもう一度ボタンを押し、URLに入力内容が付かなくなることを確かめる(確かめたら戻す)
6. `type="checkbox"` の欄と、それに結び付いた `label` を1組足して、ラベルの文字を押しても切り替わることを確かめる
7. `type="radio"` の欄を2つ、`name` を付けずに置いて両方選べてしまうことを確かめる。そのあと2つに同じ `name="slot"` を付けて、1つしか選べなくなることを確かめる

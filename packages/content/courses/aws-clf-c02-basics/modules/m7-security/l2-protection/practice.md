...
2. 次のサービスをどの段に置くか仕分けする: WAF / Shield / KMS / Secrets Manager / ACM / GuardDuty / Inspector / Macie / Security Hub / Artifact
3. 各サービスの右に「用途の一言」を書く(例: WAF=不正なリクエストを弾く、GuardDuty=怪しい動きを検出、Macie=S3の個人情報を発見)
4. 「守る」の段に、M4で学んだセキュリティグループも書き足し、WAFとの違いを1行で書く(通信の許可制御 vs 攻撃パターンの検査)
5. 最後に、余白へ「暗号化の2場面」(保存時 at rest / 転送時 in transit)と、鍵=KMS・秘密の文字列=Secrets Manager・証明書=ACMの対応を書く
...
### Q1. 「データベースの接続パスワードをコードに書かずに安全に保管し、定期的な差し替えも自動化したい」に最も適したサービスはどれですか。
...
**B** — 秘密の文字列の保管と自動ローテーションはSecrets Managerです。KMSは暗号化に使う「鍵」の管理係で、パスワードそのものを預かる金庫はSecrets Manager、と対で覚えます。
